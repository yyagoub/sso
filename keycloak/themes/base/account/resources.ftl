...
<@layout.mainLayout active='authorization' bodyClass='authorization'; section>
                ...
                <table class="table table-striped table-bordered">
                    ...
                    <#list authorization.resourcesWaitingApproval as resource>
                        <#list resource.permissions as permission>
                            <form action="${url.getResourceGrant(resource.id)}"
                                  name="approveForm-${resource.id}-${permission.requester.username}" method="post">
                                ...
                                <tr>
                                            ...
                                            <#if scope.scope??>
                                                <div class="search-box">
                                                    ...
                                                    <button class="close-icon" type="button"
                                                            ...
                                                            onclick="removeScopeElm(this.parentNode);document.forms['approveForm-${resource.id}-${permission.requester.username}']['action'].value = 'deny';document.forms['approveForm-${resource.id}-${permission.requester.username}'].submit();">
                                    ...
                                    <td width="20%" align="middle" style="vertical-align: middle">
                                        <a href="#" id="grant-${resource.name}-${permission.requester.username}"
                                           onclick="document.forms['approveForm-${resource.id}-${permission.requester.username}']['action'].value = 'grant';document.forms['approveForm-${resource.id}-${permission.requester.username}'].submit();"
                                           type="submit" class="btn btn-primary">${msg("doApprove")}</a>
                                        <a href="#" id="deny-${resource.name}-${permission.requester.username}"
                                           onclick="removeAllScopes('${resource.id}-${permission.requester.username}');document.forms['approveForm-${resource.id}-${permission.requester.username}']['action'].value = 'deny';document.forms['approveForm-${resource.id}-${permission.requester.username}'].submit();"
                                           type="submit" class="btn btn-danger">${msg("doDeny")}</a>
                                    </td>
                                </tr>
                            </form>
                        </#list>
                        ...
                        <tr>
                            ...
                            <td>
                                <#if resource.shares?size != 0>
                                    <a href="${url.getResourceDetailUrl(resource.id)}">${resource.shares?size} <i
    ...
    <div class="row">
                ...
                <table class="table table-striped table-bordered">
                    ...
                    <tr>
                        <th width="5%"><input type="checkbox"
                                              ...
                                              <#if authorization.sharedResources?size == 0>disabled="true"
                        </#if></td>
                        <th>${msg("resource")}</th>
                        <th>${msg("owner")}</th>
                        <th>${msg("application")}</th>
        ...
        <#if authorization.sharedResources?size != 0>
            <div class="col-md-12">
                <a href="#" onclick="document.forms['shareForm'].submit();" type="submit"
                   class="btn btn-danger">${msg("doRemoveSharing")}</a>
            </div>
        </#if>
    </div>
    ...
    <#if authorization.resourcesWaitingOthersApproval?size != 0>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <a href="#"
                   onclick="document.getElementById('waitingApproval').style.display=''">${msg("clickHereForDetails")}</a>
                <div class="row">
                ...
                <div class="row" id="waitingApproval" style="display:none">
                            ...
                            <table class="table table-striped table-bordered">
                                ...
                                <tr>
                                    <th width="5%"><input type="checkbox"
                                                          ...
                                                          <#if authorization.resourcesWaitingOthersApproval?size == 0>disabled="true"</#if>
                                    </th>
                                    <th>${msg("resource")}</th>
                                    <th>${msg("owner")}</th>
                                    <th>${msg("action")}</th>
                    ...
                    <div class="col-md-12">
                        <a href="#" onclick="document.forms['waitingApprovalForm'].submit();" type="submit"
                           class="btn btn-danger">${msg("doRemoveRequest")}</a>
                    </div>
                </div>
            </div>
        </div>
